...
History.updateTitle = async (id, title) => {
    const [result] = await pool.query("UPDATE historys SET title = ? WHERE id = ?", [title, id]);
    return result;
}

History.deleteIfOwner = async (id, user_id) => {
    const [result] = await pool.query("DELETE FROM historys WHERE id = ? AND user_id = ?", [id, user_id]);
